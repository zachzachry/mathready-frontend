...
function StudentTest({ studentName, onFinish }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",fontFamily:"sans-serif",background:"#e8edf2",overflow:"hidden"}}>
      <TopBar title="Grade 5 Mathematics — Practice Mode" right={
        <div style={{display:"flex",gap:"2rem",alignItems:"center"}}>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:"0.58rem",opacity:.6,letterSpacing:"0.1em"}}>TIME REMAINING</div>
            <div style={{fontSize:"1.15rem",fontWeight:"bold",fontFamily:"monospace",color:secs<300?"#ffaaaa":"#fff",letterSpacing:"0.06em"}}>{fmtTime(secs)}</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:"0.58rem",opacity:.6,letterSpacing:"0.1em"}}>STUDENT</div>
            <div style={{fontSize:"0.82rem",fontWeight:600}}>{studentName}</div>
          </div>
        </div>
      }/>
      <div style={{background:"#004e94",color:"#cce0f5",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 1.25rem",height:"30px",flexShrink:0,fontSize:"0.72rem"}}>
        <div style={{display:"flex",gap:"1.25rem"}}>
          <button onClick={()=>setNav(o=>!o)} style={{background:"none",border:"none",color:"#cce0f5",cursor:"pointer",fontSize:"0.72rem",padding:0}}>{nav?"◀ Hide Nav":"▶ Show Nav"}</button>
          <span style={{opacity:.5}}>|</span><span>{ansCount} of {TOTAL} answered</span>
          {flgCount>0&&<><span style={{opacity:.5}}>|</span><span style={{color:"#ffd166"}}>🚩 {flgCount} flagged</span></>}
        </div>
        <span style={{opacity:.65}}>No Calculator</span>
      </div>
