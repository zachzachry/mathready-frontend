...
function MathText({ text, style }) {
  ...
  // Split on $...$ math blocks first, then handle dollar signs in plain segments
  // Regex: $...$ where content contains at least one LaTeX character
  const parts = text.split(/(\$[^$]+\$)/g);

  return (
    <span style={{ whiteSpace: "pre-wrap", ...style }}>
      {parts.map((part, i) => {
        // Check if this looks like a math block
        if (part.startsWith("$") && part.endsWith("$") && part.length > 2) {
          const latex = part.slice(1, -1);
          // Only send to KaTeX if it contains LaTeX syntax
        ...
        // Plain text — render as-is (dollar signs stay as dollar signs)
