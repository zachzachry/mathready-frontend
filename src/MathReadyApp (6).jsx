...
function preprocessMath(text) {
  // Only protect money-style values: $17.50  $1,000  $50
  // NOT math expressions like $4.7 \times or $\frac
  // Money: $ followed by digits/commas, optionally .XX (1-2 decimal digits), NOT followed by LaTeX
  return text.replace(/\$(\d[\d,]*(?:\.\d{1,2})?)(?![.\d\\^_{a-zA-Z])/g, "DOLLARSIGN$1");
}
function isMathExpression(s) {
  return /[\^_{}]|frac|sqrt/.test(s);
}
    ...
    <span style={{ whiteSpace: "pre-wrap", ...style }}>
      {parts.map((part, i) => {
        if (!part.startsWith("$") || !part.endsWith("$") || part.length <= 2)
          return <span key={i}>{part.replace(/DOLLARSIGN/g, "$")}</span>;
        const latex = part.slice(1, -1);
        if (!isMathExpression(latex))
          return <span key={i}>{part.replace(/DOLLARSIGN/g, "$")}</span>;
        try {
